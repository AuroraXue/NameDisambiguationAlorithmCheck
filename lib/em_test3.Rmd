...
```{r}
#test
#dtm<-dtm_train_tfidf[1:10,]
#M_p<-constrain.mat(feature_mat = AKumar,1)[1:n,1:n]
#M_p<-MP1[1:10,1:10]

dtm<-dtm_train_tfidf
n<-nrow(dtm)
#M_p<-constrain.mat(feature_mat = AKumar,1)
M_p<-MP1
```
...
for (m in 1:M)
{
  deriv.f<-sum( D_xx_deriv * M_p * indicate )
  A[m] <- A[m] + eta * deriv.f
}
...
### result re-arrange
```{r}
mat<-cbind(tag,ori.order)
ori.tag<-mat[order(ori.order),][,1]

compare.mat<-cbind(ori.tag,true.tag=as.numeric(AKumar$AuthorID))
#compare.mat<-cbind(ori.tag,true.tag=as.numeric(AKumar$AuthorID[1:10]))
compare.mat<-compare.mat[order(compare.mat[,2]),]
tag2<-compare.mat[,1]
exist.tag<-NULL
for (i in unique(compare.mat[,2]))
{
  index<- compare.mat[,2] == i  ##find the index in the output-tag sequantially matched the true tag
  largest<-as.numeric(names(table(tag2[index])))
  largest<-largest[largest>=i][1]
  if (is.na(largest)){next}
  else{index2<- compare.mat[,1] ==  largest ## find the index in the output-tag matched 
  tag2[index2] <- i}
}

sum(compare.mat[,2]==tag2)
```
