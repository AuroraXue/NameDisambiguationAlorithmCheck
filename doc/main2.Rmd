...
After calculating the C6 constraint matrix,we followed the EM process to update tags for every paper to minimize the objective function in each iteration until the clusters stop changing.


For initialization of our EM framework, we first cluster publications into disjoint groups based on the constraints over them.
...
dtm<-tfidf_paper$AGupta.Rdata   ### change the name here
truth<-Data[[1]]$AuthorID  ### change the number here
...
M_p<-M_prod(1,tao=1,n)   ### change the number here
tag<-ini(M_p)
tag_ini<-tag
```
